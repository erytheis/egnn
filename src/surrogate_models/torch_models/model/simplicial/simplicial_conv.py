...
def _plot_comparison(batch, output_, save=None):
    import matplotlib.pyplot as plt
    from src.utils.utils import DEFAULT_COLORS as palette
    DEFAULT_BG_COLOR = "#E7EFED"

    edge_mask = batch.mask_by_features(value=0, column_idx=2)
    # node_mask = batch.mask_by_features(value=1, column_idx=2, attribute='x')
    junction_nodes = batch.x[:, 2] == 1
    loss_coefficient = batch.edge_attr[:, 1][edge_mask].cpu()

    flowrates_predicted = output_['edge_attr'][edge_mask].squeeze().cpu()
    dH_predicted = torch.abs(flowrates_predicted) ** 1.852 * loss_coefficient * torch.sign(flowrates_predicted)

    # heads = batch.x[:, 1]
    heads = batch.node_y
    flowrates = batch.edge_y[edge_mask].squeeze().cpu()
    dH_true = heads[batch.edge_index[1][edge_mask]] - heads[batch.edge_index[0][edge_mask]]

    H_min_max = [heads.cpu().min(), heads.cpu().max()]
    Q_min_max = [flowrates.min(), flowrates.max()]
    dH_min_max = [dH_predicted.min(), dH_predicted.max()]

    fig, axs = plt.subplots(1, 3, figsize=(12, 4))

    # set options
    scatterplot_options = {'alpha': 0.5}
    line_options = {'linestyle': ':', 'c': "#555555"}
    subtitle_font = {'fontname': 'Helvetica'}

    # setting for hte axis
    for ax in axs:
        ...
        ax.grid(c='w', linewidth=2)
        ax.set_axisbelow(True)

    axs[0].scatter(heads[junction_nodes].cpu(), output_['x'][junction_nodes].cpu(),
                   **{'c': palette[0], 'edgecolors': palette[2], }, **scatterplot_options)
    axs[0].plot(H_min_max, H_min_max, **line_options)
    axs[0].set_xlabel('EPANET heads, $m$')
    axs[0].set_ylabel('Predicted heads, $m$')

    axs[1].scatter(dH_true.cpu(), dH_predicted, **{'c': palette[-2], 'edgecolors': palette[4], }, **scatterplot_options)
    axs[1].plot(dH_min_max, dH_min_max, palette[2], **line_options)
    axs[1].set_xlabel('EPANET headlosses $m$')
    axs[1].set_ylabel('Predicted headlosses $m$')

    axs[2].scatter(flowrates, flowrates_predicted, **{'c': palette[-1], 'edgecolors': palette[-2], },
                   **scatterplot_options)
    axs[2].plot(Q_min_max, Q_min_max, **line_options)
    axs[2].set_xlabel('EPANET flows, $m^3/s$', **subtitle_font)
    axs[2].set_ylabel('Predicted flows, $m^3/s$', **subtitle_font)
